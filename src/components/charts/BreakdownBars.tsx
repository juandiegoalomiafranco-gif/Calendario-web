...
export interface BreakdownRow {
  ...
  label: string
  count: number
  /** Clase de color de la barra, p. ej. `bg-act-run`. */
  colorClass: string
  /** Clase del fondo suave del icono, p. ej. `bg-act-soft-run text-act-run`. */
  toneClass?: string
}
        ...
        <div key={r.key} className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="mb-1 flex justify-between gap-2 text-sm">
              <span className="truncate font-medium text-content">{r.label}</span>
