...
export function TrendLine({ points, unit, color = 'rgb(var(--accent))', decimals = 1 }: TrendLineProps) {
  ...
  return (
    <div className="rounded-3xl border border-line bg-surface p-4 shadow-card">
        ...
        {points.map((p, i) => (
          <g key={`${p.label}-${i}`}>
            ...
            {(i % showEvery === 0 || i === points.length - 1) && (
              <>
                <text x={x(i)} y={y(p.value) - 9} textAnchor="middle" fontSize={11} fontWeight={600} fill="rgb(var(--text))">
                  {p.value.toFixed(decimals)}
                </text>
                <text x={x(i)} y={H - 6} textAnchor="middle" fontSize={10} fill="rgb(var(--text-subtle))">
                  {p.label}
                </text>
              </>
            )}
      ...
      {unit && <p className="text-[11px] text-content-subtle text-right mt-1">{unit}</p>}
    </div>
  )
}
