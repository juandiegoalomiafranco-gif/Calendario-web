  ...
  return (
    <div className="flex flex-col gap-2">
      {days.map((day) => {
        ...
        const trainings = day.sessions.filter((s) => s.type !== 'rest')
        const allDone = trainings.length > 0 && trainings.every((s) => log[s.id]?.completed)
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0 flex flex-wrap gap-1.5">
              {holiday && <HolidayBadge name={holiday} short />}
              {day.sessions.map((s) => {
                ...
                return (
                  ...
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${done ? 'bg-ok-500' : meta.dot}`} />
                    {meta.emoji} {s.summary}
                    {done && <span aria-label="completada">✓</span>}
                  </span>
                )
              })}
            </div>
